...
**AI agent sessions (Claude Code & co) — direnv never fires there.** An agent's shell is non-interactive: direnv hooks don't run, so a project's devShell and `.envrc` exports are NOT active inside the agent's commands even with the `.envrc` right there. The agent must enter environments explicitly — `direnv exec <project-root> <cmd>` (full `.envrc` contract) or `nix develop <project-root> -c <cmd>` (devShell only). Put that rule in your global `~/.claude/CLAUDE.md` and manage that file from your env repo (write it as `home/claude-user-CLAUDE.md`, symlink into place at switch) so every session starts knowing it.
